...
router.get("/reviewdetails/:idReview", async (req, res) => {
  ...
  if (
    ...
    // Check if currentUser is an object and not empty and has properties
    isLoggedIn = true;
  }
  ...
  if (currentUser && currentUser.userID) {
    // Only check for interaction if currentUser has a userID
    interaction = await review.interactions.find(
      (interaction) => interaction.userId === currentUser.userID,
    );
    if (interaction) {
  ...
  if (
    currentUser &&
    currentUser.isEstablishmentOwner &&
    currentUser.establishmentID === review.restoID
  ) {
...
router.get("/editlogout/:currentUserID", async (req, res) => {
  const currentUserID = req.params.currentUserID;
  const currentUser = await User.findOne({ userID: currentUserID });
  const currentUserReviewCount = (await Review.find({ userID: currentUserID }))
    .length;
  res.render("editlogout", {
    css: ["styles_g", "styles2"],
    currentUser,
...
router.post("/editprofile/:currentUserID", upload, async (req, res) => {
  try {
    const currentUserID = req.params.currentUserID;
    const { userName, userPassword, userDesc, currentPassword, fileInput } =
      req.body;
...
router.post("/reviews/helpful", async (req, res) => {
  ...
  try {
    ...
    const userInteraction = review.interactions.find(
...
router.post("/reviews/removehelpful", async (req, res) => {
  ...
  try {
    ...
    //Remove Interaction
    const index = review.interactions.findIndex(
      (interaction) => interaction.userId === userId,
    );
    if (index !== -1) {
...
router.post("/submit-review", async (req, res) => {
  try {
    ...
    if (reviewContent.length > 150) {
      return res
        .status(400)
        .send("Review content must be 150 characters or less");
    }
...
router.post("/reviews/respond", async (req, res) => {
  ...
  try {
    ...
    const latestResponse =
      updatedReview.responses[updatedReview.responses.length - 1];
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      const restaurant = await Resto.findOneAndDelete({
        restoID: req.params.id,
      });
